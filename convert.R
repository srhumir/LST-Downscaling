...
convert <- function(SpotNDVI, min, max, minLST, maxLST, nn, lm, pol, ncell){
...
#+              pol$coefficients[3]* modisLike[5]^2
       if (sum(is.na(modisLike)) > 0){
              modisLike <- fillNA(modisLike)
       }
       #start nn. scale the modislike
       scaled <- scale(modisLike, center = minLST, scale = maxLST - minLST)
       #run nn
